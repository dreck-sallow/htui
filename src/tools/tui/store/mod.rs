...
use project::{Project, StoreMapping, StoreMappingItem};
pub mod project;
...
impl Store {
    ...
    pub fn get_or_create_project(&self, name: &str) -> Option<Project> {
        if let Ok(mut mapping) = self.mapping_store.load_mapping() {
            return match mapping.find_by_name(name) {
                Some(itm) => self.projects_store.load_project(itm.id()).ok(),
                None => {
                    let mapping_item = StoreMappingItem::new(name);
                    let mapping_item_id = mapping_item.id().to_owned();

                    mapping.projects.push(mapping_item);
                    self.mapping_store.save_mapping(&mapping).ok()?;

                    let project = Project::new(name);

                    self.projects_store
                        .save_project(&mapping_item_id, &project)
                        .ok()?;
                    Some(project)
                }
            };
...
impl MappingStore {
    ...
    pub fn load_mapping(&self) -> Result<StoreMapping, StoreError> {
        ...
        // If the file is empty, fill the file with default data
        if content.trim().is_empty() {
            let store_mapping = StoreMapping::default();
            self.save_mapping(&store_mapping)?;
            Ok(store_mapping)
        } else {
            let store_mapping = toml::from_str(&content);
            Ok(store_mapping?)
        }
    }
